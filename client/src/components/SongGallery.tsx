...
interface SongGalleryProps {
  ...
  initialFilterMoment?: CeremonyMoment | null;
  isModalMode?: boolean;
}
...
export default function SongGallery({ 
  onSelectMoment, 
  selectedMoments,
  initialFilterMoment = null,
  isModalMode = false 
}: SongGalleryProps) {
  const [currentFilter, setCurrentFilter] = useState<CeremonyMoment | 'all'>(
    initialFilterMoment || 'all'
  );
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedSong, setSelectedSong] = useState<Song | null>(null);
  const [lyricsModalOpen, setLyricsModalOpen] = useState(false);

  // Update filter when initialFilterMoment changes
  useEffect(() => {
    if (initialFilterMoment) {
      setCurrentFilter(initialFilterMoment);
    }
  }, [initialFilterMoment]);
  ...
  const filteredSongs = useMemo(() => {
    return songs.filter(song => {
      // If in modal mode or filtering by a specific moment, only show songs suitable for that moment
      const matchesFilter = 
        currentFilter === 'all' 
          ? true 
          : song.suitableMoments.includes(currentFilter as CeremonyMoment);
  ...
  return (
    <div className={`bg-white ${!isModalMode ? 'rounded-lg shadow-md p-6' : ''}`}>
      ...
      {(!isModalMode || currentFilter === 'all') && (
        <div className="mb-6">
          <div className="flex flex-wrap gap-3 mb-4">
            {/* Don't show all the tabs in modal mode */}
            {(isModalMode ? [currentFilter] : ceremonyMoments).map((moment) => (
              <Button
                key={moment}
      ...
      {isLoading ? (
        ...
        <div className={`grid grid-cols-1 ${isModalMode ? 'sm:grid-cols-1' : 'md:grid-cols-2'} gap-4`} id="song-gallery">
          {filteredSongs.map((song) => (
            <SongCard
              ...
              highlightMoment={isModalMode ? (currentFilter as CeremonyMoment) : undefined}
            />
          ))}
